...
import { Briefcase, DollarSign, Clock, MapPin, Award, Star, Edit2, Save, X, AlertCircle } from 'lucide-react'
...
export default function HelperServicesPage() {
  ...
  const cancelEdit = () => {
    if (profile) {
      setEditedHourlyRate(profile.hourly_rate)
  ...
  const addServiceArea = () => {
    if (newServiceArea.trim() && !editedServiceAreas.includes(newServiceArea.trim())) {
      setEditedServiceAreas([...editedServiceAreas, newServiceArea.trim()])
      setNewServiceArea('')
    }
  }

  const removeServiceArea = (area: string) => {
    setEditedServiceAreas(editedServiceAreas.filter(a => a !== area))
  }
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-white/50 shadow-lg">
              ...
              <CardContent>
                ...
                <p className="text-sm text-gray-500 mt-3">
                  To change service categories, contact support or complete onboarding again
                </p>
              </CardContent>
            </Card>
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-white/50 shadow-lg">
                ...
                <CardContent>
                  {editing ? (
                    ...
                    <p className="text-3xl font-bold text-gray-900">₹{profile.hourly_rate}/hour</p>
                  )}
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2 mb-3">
                  {editedServiceAreas.map((area, idx) => (
                    <span key={idx} className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-sm font-medium flex items-center gap-2">
                      {area}
                      {editing && (
                        <button onClick={() => removeServiceArea(area)} className="text-red-900 hover:text-red-600">
                          <X className="h-3 w-3" />
                        </button>
                      )}
                    </span>
                ...
                {editing && (
                  <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={addServiceArea} size="sm">Add</Button>
                  </div>
                )}
              </CardContent>
            </Card>
