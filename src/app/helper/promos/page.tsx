...
export default async function HelperPromosPage() {
  ...
  // Display referral code (generate if missing)
      ...
      <div className='mx-auto max-w-5xl space-y-6'>
        ...
        <div className='grid gap-6 md:grid-cols-2'>
          <div className='space-y-2'>
            <h2 className='font-semibold text-sm'>Referral Conversions</h2>
            {(referrals || []).map(r => (
              <div key={r.id} className='rounded border p-2 bg-white text-xs flex justify-between'>
                <span>{r.status}</span>
                <span>{new Date(r.created_at).toLocaleDateString()}</span>
              </div>
            ))}
            {(!referrals || referrals.length === 0) && <p className='text-xs text-muted-foreground'>No referrals yet.</p>}
          </div>
          <div className='space-y-2'>
