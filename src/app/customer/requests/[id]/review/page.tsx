...
export default function CustomerReviewPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: helper } = await supabase
        .from('profiles')
        .select('full_name')
        .eq('id', reqTyped.assigned_helper_id)
        .maybeSingle()
      if (helper) setHelperName((helper as any).full_name || 'Helper')
  ...
  const submit = async () => {
    ...
    if (!helperId) { setError('Missing helper'); setSubmitting(false); return }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          <div className="w-24 h-24 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center animate-bounce">
            <CheckCircle className="w-12 h-12 text-white" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mb-3">Thank You! 🎉</h2>
          <p className="text-gray-600 mb-2">Your review has been submitted successfully.</p>
          <p className="text-sm text-emerald-600">Redirecting you back...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="mx-auto max-w-lg">
        ...
        {error ? (
          ...
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-gray-600">You have already submitted a review for this service.</p>
            <Link 
              href={`/customer/requests/${requestId}`}
              className="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-emerald-500 text-white rounded-xl font-semibold hover:bg-emerald-600 transition-colors"
            >
              Go Back
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {/* Header Card */}
            <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-600 rounded-3xl p-8 text-white text-center">
              ...
              <div className="relative">
                ...
                <p className="text-emerald-100">
                  How was your service with {helperName}?
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              ...
              {/* Comment Textarea */}
              <textarea 
                className="w-full min-h-[120px] p-4 bg-gray-50 border-0 rounded-2xl text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all resize-none"
            ...
            <button
              onClick={submit}
              disabled={submitting}
              className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-2xl font-bold text-lg shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {submitting ? (
                <>
            ...
            <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100">
              ...
              <p className="text-sm text-amber-800">
                Your honest feedback helps {helperName} improve and helps other customers make informed decisions.
              </p>
            </div>
          </div>
        )}
